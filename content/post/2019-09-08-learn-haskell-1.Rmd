...
```{haskell engine.path='/Users/duliu/.ghcup/bin/ghc'}
putStrLn "Hello, world!"
```


This doesn't look any different to other programming languages, but the mind blowing thing is, `putStrLn` is actually not causing any _side effect_. But wait, if it doesn't cause side effect, how come it can __print__ words onto the screen? Well, it turns out that `print` takes an argument, "Hello, world!" in this case, and returns an `IO ()` which takes care of the side effecting. That is, the type of `putStrLn` is `String -> IO ()` (equivalently, `[Char] -> IO ()`).

```{haskell engine.path='/Users/duliu/.ghcup/bin/ghc'}
# Haskell is statically typed with type erasure, therefore we need to use toDyn to turn them into dynamic types
import Data.Dynamic
print (dynTypeRep (toDyn putStrLn))
```
